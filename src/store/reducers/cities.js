...
const city_reducer = createReducer(
    ...
    builder => builder.addCase( //callback constructora de estados globales (Agrega minimo un caso de reduccion para cada accion)
            ...
            (state,action) => {//callback que depende del estado y la accion y es la encargada de reducir los estados
                    let new_state = {
                        ...state,  //a la copia del estado  tengo que "llenarle" la propiedad carousel con los datos que me envia la accion
                        carousel: action.payload.carousel
                    
                    }
                    return new_state //retorno el nueevo estado para que se actualice la vista
